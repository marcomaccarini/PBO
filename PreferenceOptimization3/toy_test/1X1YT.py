...
def point_eval_out1(point):
    x = point[0]
    r = (2 * np.sin(20 * x)) / 10 + np.exp((1 - (5 * x) / 8) ** 2)
    return r
...
def my_pref(x1, x2, f=None):
    ...
    if f is not None:
        ...
        print("best is :  %20s, next is: %20s, f(best): %4.4f, f(next): %4.4f, user's input: %d" % (
            str(x1), str(x2), round(f(x1)[0], 3), round(f(x2)[0], 3), Y_pref))
        return Y_pref
    else:
...
my_problem = GL.GLISpFinale(fvarsX, fvarsY, my_pref, objectives=objectives, objectivesFunctions=objectivesFunctions,
                            acquisition_optimizer_type='lbfgsb', kfold=3, delta=1.2, objective=(10, 1),
                            batch_size=1, init_n_samples=init_samples, save_experiment=False, plot=False,
                            title="Bemporad Function", save=True, max_bound=bb, name_opt='presentazione',
